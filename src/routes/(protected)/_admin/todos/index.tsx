...
function TodosRoute() {
  ...
  const todos = useSuspenseQuery(todosListQueryOptions(search)).data;
  const users = useSuspenseQuery(usersDirectoryQueryOptions()).data.users;
  const onInputChange = (next: Partial<TodosListInput>) =>
    void navigate({ search: (previous) => ({ ...previous, ...next }) });
  return <TodosPage data={todos} input={search} users={users} onInputChange={onInputChange} />;
}
