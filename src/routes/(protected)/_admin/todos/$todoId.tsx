...
export const Route = createFileRoute("/(protected)/_admin/todos/$todoId")({
  loader: ({ context, params }) => {
    const todoId = parseNumericId(params.todoId, "task id");
    return Promise.all([
...
function TodoRoute() {
  ...
  const todo = useSuspenseQuery(todoDetailQueryOptions(todoId)).data;
  const users = useSuspenseQuery(usersDirectoryQueryOptions()).data.users;
  const assignee = users.find((user) => user.id === todo.userId);
  return (
    <TodoDetailPage
      todo={todo}
      assignee={assignee ? `${assignee.firstName} ${assignee.lastName}` : `User #${todo.userId}`}
    />
  );
}
