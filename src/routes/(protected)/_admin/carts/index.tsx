...
export const Route = createFileRoute("/(protected)/_admin/carts/")({
  ...
  loader: ({ context, deps }) => Promise.all([context.queryClient.ensureQueryData(cartsListQueryOptions(deps)), context.queryClient.ensureQueryData(usersDirectoryQueryOptions())]),
...
function CartsRoute() { const search = Route.useSearch(); const navigate = Route.useNavigate(); const carts = useSuspenseQuery(cartsListQueryOptions(search)).data; const users = useSuspenseQuery(usersDirectoryQueryOptions()).data.users; const onInputChange = (next: Partial<CartsListInput>) => void navigate({ search: (previous) => ({ ...previous, ...next }) }); return <CartsPage data={carts} input={search} users={users} onInputChange={onInputChange} />; }
