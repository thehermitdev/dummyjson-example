...
function replaceTodo(queryClient: QueryClient, todo: Todo) {
  queryClient.setQueriesData<TodosListResponse>({ queryKey: todosKeys.lists() }, (current) =>
    current
      ? { ...current, todos: current.todos.map((item) => (item.id === todo.id ? todo : item)) }
      : current,
  );
    ...
    onSuccess: (todo) => {
      queryClient.setQueriesData<TodosListResponse>({ queryKey: todosKeys.lists() }, (current) =>
        current
          ? {
              ...current,
              todos: [todo, ...current.todos].slice(0, current.limit || current.todos.length + 1),
...
export function deleteTodoMutationOptions(queryClient: QueryClient) {
  return mutationOptions({
    ...
    onSuccess: (todo) => {
      ...
      queryClient.removeQueries({ queryKey: todosKeys.detail(todo.id) });
    },
  });
}
