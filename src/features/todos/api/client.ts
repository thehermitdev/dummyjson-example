import { createTodoInputSchema, deletedTodoSchema, todoSchema, todosListResponseSchema, updateTodoInputSchema } from "./contracts";
import type { CreateTodoInput, Todo, TodosListResponse, UpdateTodoInput } from "./contracts";
import { httpClient } from "#/shared/api/http-client";
import { ApplicationError } from "#/shared/errors/application-error";

export interface TodosListInput { page: number; pageSize: number; userId?: number | undefined }
...
export async function getTodos(input: TodosListInput, signal?: AbortSignal): Promise<TodosListResponse> {
  ...
  if (!result.success) throw new ApplicationError("Invalid todos response", { code: "API_CONTRACT_ERROR", details: result.error.flatten(), cause: result.error });
  return result.data;
}
...
export async function getTodo(todoId: number, signal?: AbortSignal): Promise<Todo> {
  ...
  if (!result.success) throw new ApplicationError("Invalid todo response", { code: "API_CONTRACT_ERROR", details: result.error.flatten(), cause: result.error });
  return result.data;
}
...
export async function addTodo(input: CreateTodoInput): Promise<Todo> {
  ...
  return todoSchema.parse({ ...parsed, ...(typeof response.data === "object" && response.data ? response.data : {}) });
}
...
export async function updateTodo(todo: Todo, input: UpdateTodoInput): Promise<Todo> {
  ...
  return todoSchema.parse({ ...todo, ...parsed, ...(typeof response.data === "object" && response.data ? response.data : {}) });
}
