...
export async function getCarts(input: CartsListInput, signal?: AbortSignal): Promise<CartsListResponse> {
  ...
  if (!result.success) throw new ApplicationError("Invalid carts response", { code: "API_CONTRACT_ERROR", details: result.error.flatten(), cause: result.error });
  return result.data;
}
...
export async function getCart(cartId: number, signal?: AbortSignal): Promise<Cart> {
  ...
  if (!result.success) throw new ApplicationError("Invalid cart response", { code: "API_CONTRACT_ERROR", details: result.error.flatten(), cause: result.error });
  return result.data;
}
