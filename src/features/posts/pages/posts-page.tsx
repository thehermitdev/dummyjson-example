    ...
    try {
      if (editing) {
        ...
        toast.success("Post created", {
          description: "The simulated post was inserted into the current cache.",
        });
      }
      setDrawerOpen(false);
    } catch (error) {
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      toast.error("Delete failed", {
        description: error instanceof Error ? error.message : "Unable to delete post.",
      });
    }
  };
      ...
      <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div>
          ...
          <p className="mt-2 text-sm text-muted-foreground">
            Manage user-generated content, tags, authors, reactions, and comments.
          </p>
        </div>
      ...
      <div className="overflow-hidden rounded-xl border bg-card shadow-xs">
        <div className="grid gap-3 border-b p-4 xl:grid-cols-[minmax(220px,1fr)_170px_200px_150px_130px_auto]">
          ...
          <select
            ...
            onChange={(event) =>
              onInputChange({
                sortBy: (event.target.value || undefined) as PostsListInput["sortBy"],
                page: 1,
              })
            }
            className="h-9 rounded-md border bg-background px-3 text-sm"
              ...
              {data.posts.map((post) => (
                <tr key={post.id} className="hover:bg-muted/30">
                  ...
                  <td className="px-4 py-3 text-muted-foreground">
                    {userMap.get(post.userId) ?? `User #${post.userId}`}
                  </td>
                  ...
                  <td className="px-4 py-3 text-xs text-muted-foreground">
                    {post.views.toLocaleString()} views · {post.reactions.likes} likes
                  </td>
                  <td className="px-4 py-3">
